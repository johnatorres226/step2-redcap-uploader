...
class DataProcessor:
    ...
    def standardize_redcap_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        redcap_fields = {"redcap_event_name": "", "redcap_repeat_instrument": "", "redcap_repeat_instance": ""}
        ...
        if "redcap_repeat_instance" in df_std.columns:
            df_std["redcap_repeat_instance"] = pd.to_numeric(df_std["redcap_repeat_instance"], errors="coerce").fillna(
                ""
            )
    ...
    def validate_against_metadata(self, df: pd.DataFrame, metadata: List[Dict[str, Any]]) -> bool:
    ...
    def _validate_choice_field(self, df: pd.DataFrame, column: str, field_info: Dict) -> bool:
        ...
        if valid_choices:
            invalid_values = df[df[column].notna() & ~df[column].astype(str).isin(valid_choices)]
    ...
    def _validate_yesno_field(self, df: pd.DataFrame, column: str) -> bool:
        ...
        invalid_values = df[df[column].notna() & ~df[column].isin(valid_values)]
    ...
    def add_audit_trail(
        self, upload_data: List[Dict[str, Any]], current_redcap_data: List[Dict[str, Any]], user_initials: str
    ) -> List[Dict[str, Any]]:
        """
