...
class QCDataUploader:
    ...
    def upload_qc_status_data(
        ...
        try:
            ...
            if not json_files:
                ...
                return {"success": False, "error": error_msg, "output_directory": str(output_dir)}
            ...
            upload_data_with_audit = self.data_processor.add_audit_trail(upload_data, current_data, initials)
            ...
            if not dry_run:
                ...
                if upload_result["success"]:
                    ...
                    return {"success": False, "error": upload_result["error"], "output_directory": str(output_dir)}
            else:
    ...
    def upload_query_resolution_data(self, data_file: Path, initials: str, dry_run: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not file_result["success"]:
                return {"success": False, "error": file_result["error"], "output_directory": str(output_dir)}
    ...
    def _load_json_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "data": records, "file_path": str(file_path), "record_count": len(records)}
            ...
            for i, record in enumerate(data):
                ...
                if record_errors:
                    validation_errors.append(
                        {"record_index": i, "record_id": record.get("ptid", "Unknown"), "errors": record_errors}
                    )
    ...
    def _get_record_identity(self, record: Dict[str, Any]) -> tuple[str, str, str, str]:
        ...
        repeat_instance = str(record.get("redcap_repeat_instance") or record.get("redcap_event_instance") or "")
    ...
    def _create_backup_data(
        self, original_data: List[Dict[str, Any]], new_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            backup_data = {
                ...
                "backup_metadata": {"created_by": "QCDataUploader", "purpose": "Fallback for data upload"},
            }
    ...
    def _filter_new_records(
        self, new_data: List[Dict[str, Any]], current_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _upload_to_redcap(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Make API request
            response = self.session.post(self.config.api_url, data=request_data, timeout=self.config.timeout)
            ...
            self.logger.error(error_msg)
            return {"success": False, "error": error_msg, "error_type": "request_failed"}

        except Exception as e:
            error_msg = f"Unexpected error during upload: {str(e)}"
            self.logger.error(error_msg)
            return {"success": False, "error": error_msg, "error_type": "unexpected"}

    def _track_upload(self, upload_type: str, file_paths: List[str], initials: str, records_count: int) -> None:
